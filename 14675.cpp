...
// 단절점 !
/*
 * 단절점의 특성을 알아보자. 무식하게 노드 없애고 그래프 특성 확인하면 시간 초과 ! N: 노드개수 M: 에지수 --> O(N(N+M))
 * 단절점의 특성은, 단절점과 연결된 점들이 단절점을 제외하면 우회로가 없다.
 * DFS를 이용하여 정점들의 방문 순서를 기록해주자
 * 특정 A번 정점에서 자식 노드들이 정점 A를 거치지 않고 정점 A보다 빠른 방문번호를 가진 정점으로 갈 수 없다면
 * A는 단절점이다 !
